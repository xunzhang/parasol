...
class mf(paralg):
	...
	for it in xrange(self.rounds):
		...
		# learning
	        ...
	        delta_p = self.alpha * (2 * eij * self.q[:, j] - self.beta * self.p[i, :])
		delta_q = self.alpha * (2 * eij * self.p[i, :] - self.beta * self.q[:, j])
		
		# push updates
		paralg.paralg_inc(self, key_p, delta_p / self.b)
		paralg.paralg_inc(self, key_q, delta_q / self.a)
	...
	paralg.set_phases(self, self.rounds * len(self.graph))
	paralg.sync(self)
        s = time.time()
